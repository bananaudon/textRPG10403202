public class itemBag {
    ...
    public void setItem(int bagNumber, item Item) {
        bags[bagNumber] = Item;
        System.out.println(bagNumber + "番に" + Item.itemName + "を入れたよ このコメントはitemBagクラスのsetItemメソッド");
    }
    ...
    public void addItem(item Item) {
        for (int i = 0; i < bags.length; i++) {
            if (bags[i] instanceof item_void) {
                bags[i] = Item;
                System.out.println(i + "番に" + Item.itemName + "を入れたよ このコメントはitemBagクラスのaddItemメソッド");
                break;
            }
        }
    }
